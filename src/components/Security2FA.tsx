...
import {
  Shield, ShieldCheck, Smartphone, Monitor, Globe, Clock, MapPin,
  Fingerprint, LogOut, AlertTriangle, CheckCircle2, XCircle, Eye, KeyRound,
} from 'lucide-react';
...
const auditLog = [
  { action: 'Login Attempt', details: 'Successful login via Chrome', time: '2 min ago', status: 'success' },
  { action: 'Password Change', details: 'Password updated successfully', time: '1 hour ago', status: 'success' },
  { action: 'API Key Generated', details: 'New API key "ZC-PROD-01" created', time: '3 hours ago', status: 'success' },
  { action: 'Login Attempt', details: 'Failed login from 203.45.67.89', time: '5 hours ago', status: 'failed' },
  { action: '2FA Disabled', details: 'Two-factor authentication disabled', time: '1 day ago', status: 'warning' },
  { action: 'Session Revoked', details: 'Session sess_04 terminated', time: '1 day ago', status: 'success' },
  { action: 'Withdrawal Approved', details: 'BTC 0.25 withdrawal processed', time: '2 days ago', status: 'success' },
  { action: 'Login Attempt', details: 'Suspicious login blocked — VPN detected', time: '2 days ago', status: 'failed' },
  { action: 'API Key Rotated', details: 'API key "ZC-STAGE-02" rotated', time: '3 days ago', status: 'success' },
  { action: 'Account Locked', details: 'Auto-locked after 5 failed attempts', time: '4 days ago', status: 'warning' },
];
...
export default function Security2FA() {
  ...
  const handleVerify = () => {
    ...
    setTimeout(() => {
      ...
      setTimeout(() => { setDialogOpen(false); setVerifySuccess(false); setOtpValue(''); }, 1500);
    }, 2000);
  };

  const handleRevoke = (id: string) => { console.log('Revoking session:', id); };
  ...
  return (
    <div className="space-y-8 p-6 animate-fade-in-up">
      <div>
        ...
        <p className="text-muted-foreground mt-1">Manage your account security, sessions, and audit trail</p>
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="glass-card p-8 flex flex-col items-center justify-center animate-scale-in">
          <div className="relative">
            <svg width="200" height="200" viewBox="0 0 200 200">
              ...
              <circle cx="100" cy="100" r={RADIUS} fill="none" stroke="url(#scoreGradient)" strokeLinecap="round" strokeDasharray={CIRCUMFERENCE} strokeDashoffset={STROKE_OFFSET} transform="rotate(-90 100 100)" className="transition-all duration-1000 ease-out" />
            </svg>
          ...
          <div className="mt-4 text-center">
            ...
            <p className="text-sm text-muted-foreground mt-1">Good — Enable 2FA to improve your score</p>
          </div>
        </div>
        ...
        <div className="glass-card p-8 animate-scale-in" style={{ animationDelay: '0.1s' }}>
          <div className="flex items-center justify-between mb-6">
            ...
            <Badge variant={twoFAEnabled ? 'default' : 'secondary'} className={twoFAEnabled ? 'bg-emerald-500/20 text-emerald-400 border-emerald-500/30' : 'bg-red-500/15 text-red-400 border-red-500/30'}>
          ...
          <div className="space-y-4">
            ...
            <div className="flex items-start gap-3 p-3 rounded-lg bg-white/[0.03]">
              ...
              <div>
                ...
                <p className="text-xs text-muted-foreground">Receive codes via SMS as fallback</p>
              </div>
            </div>
            <div className="flex items-start gap-3 p-3 rounded-lg bg-white/[0.03]">
              <Fingerprint className="h-5 w-5 text-violet-400 mt-0.5 shrink-0" />
              <div>
                <p className="text-sm font-medium text-foreground">Hardware Key</p>
                <p className="text-xs text-muted-foreground">Use YubiKey or similar (Pro plan)</p>
              </div>
            </div>
          </div>
          <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
            ...
            <DialogContent className="glass-card border-white/10 sm:max-w-md">
              <DialogHeader>
                ...
                <DialogDescription className="text-muted-foreground">Scan the QR code with your authenticator app and enter the 6-digit code</DialogDescription>
              </DialogHeader>
              <div className="space-y-6 py-4">
                  ...
                  <div className="relative w-48 h-48 bg-white rounded-lg p-3">
                    <div className="w-full h-full grid grid-cols-8 grid-rows-8 gap-0.5">
                      {Array.from({ length: 64 }).map((_, i) => (
                        <div key={i} className={`rounded-[1px] ${Math.random() > 0.45 ? 'bg-gray-900' : 'bg-white'}`} />
                      ))}
                    </div>
                ...
                <p className="text-center text-xs text-muted-foreground">Scan with Google Authenticator or Authy</p>
                {verifySuccess ? (
                  <div className="flex items-center justify-center gap-2 text-emerald-400 animate-scale-in">
                  ...
                  <>
                    ...
                    <Button onClick={handleVerify} disabled={otpValue.length < 6 || verifying} className="w-full gradient-bg-1 text-foreground font-semibold hover:opacity-90 transition-opacity">
      ...
      <div className="glass-card p-6 animate-fade-in-up" style={{ animationDelay: '0.15s' }}>
        ...
        <div className="space-y-3 stagger-children">
          {loginActivity.map((login, i) => (
            <div key={i} className="flex items-center gap-4 p-4 rounded-lg bg-white/[0.03] hover:bg-white/[0.05] transition-colors">
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2">
                  <span className="text-sm font-medium text-foreground">{login.device}</span>
                  <span className="text-xs text-muted-foreground">· {login.browser}</span>
                  {login.current && <Badge className="bg-emerald-500/20 text-emerald-400 border-emerald-500/30 text-[10px] px-1.5">Current</Badge>}
                </div>
      ...
      <div className="glass-card p-6 animate-fade-in-up" style={{ animationDelay: '0.2s' }}>
        ...
        <div className="space-y-3">
          {activeSessions.map((session) => (
            <div key={session.id} className="flex items-center gap-4 p-4 rounded-lg bg-white/[0.03] hover:bg-white/[0.05] transition-colors">
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2">
              ...
              <Button variant="ghost" size="sm" onClick={() => handleRevoke(session.id)} className="text-red-400 hover:text-red-300 hover:bg-red-500/10 shrink-0">
            ...
            <TableBody>
              {auditLog.map((entry, i) => (
                <TableRow key={i} className="border-white/[0.04] hover:bg-white/[0.03]">
                  ...
                  <TableCell className="text-sm text-muted-foreground whitespace-nowrap">{entry.time}</TableCell>
                  <TableCell className="text-right">
                    <div className="flex items-center justify-end gap-1.5">
                      {statusIcon(entry.status)}
                      <span className={`text-xs font-medium capitalize ${entry.status === 'success' ? 'text-emerald-400' : entry.status === 'failed' ? 'text-red-400' : 'text-amber-400'}`}>
                        {entry.status}
                      </span>
                    </div>
                  </TableCell>
