...
const DEFAULT_LOGS: AlertLog[] = [
  {
    ...
    ruleId: 'r1',
    symbol: 'BTC',
    message: 'Price alert: BTC crossed above $105,000',
  ...
  },
  {
    ...
    ruleId: 'r2',
    symbol: 'ETH',
    message: 'Rule created: Buy ETH below $2,400',
...
export default function TradeAutomation() {
  ...
  // Simulate rule triggers every 5 seconds
  useEffect(() => {
    const interval = setInterval(() => {
      setRules((prev) => {
        ...
        prev.forEach((r) => {
          ...
          if (!asset) return;
          const triggered =
            (r.condition === 'ABOVE' && asset.price > r.targetPrice) ||
            (r.condition === 'BELOW' && asset.price < r.targetPrice) ||
            ((r.condition === 'CROSS_ABOVE' || r.condition === 'CROSS_BELOW') &&
              Math.random() < 0.02);

          if (triggered) {
            updated = updated.map((rule) =>
              rule.id === r.id
                ? { ...rule, triggered: true, triggeredAt: new Date().toISOString() }
                : rule
            );
            setLogs((prevLogs) => [
              {
  ...
  return (
    <div className="animate-fade-in-up space-y-6">
            ...
            <div className="glass-card-3d p-5 border-l-2 border-l-amber-500">
              ...
              <div className="mt-4 flex gap-3">
                <Button
                  onClick={addRule}
                  disabled={!formPrice}
                  className="gradient-bg-2 text-black font-semibold"
                >
      ...
      {/* Two-column layout: Active Rules + Alert History */}
                    ...
                    <div className="flex items-start justify-between mb-2">
                      <div className="flex items-center gap-2">
                        ...
                        <Badge
                          ...
                          variant={
                            rule.action === 'SELL' ? 'destructive' : 'secondary'
                          }
                        >
                          {rule.action}
                        </Badge>
