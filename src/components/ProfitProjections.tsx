...
export default function ProfitProjections() {
  ...
  const curves = useMemo(() => {
    ...
    return allScenarios.map((s) => {
      const curve = projectDCA(parsed.initial, parsed.monthly, parsed.months, s.annualRate);
      return {
        ...s,
        curve,
        finalValue: curve[parsed.months],
      };
    });
  }, [parsed]);

  const totalInvested = parsed.initial + parsed.monthly * parsed.months;
  const maxValue = Math.max(...curves.flatMap((c) => c.curve), totalInvested, 1);
  ...
  const xTicks = Array.from(
    { length: Math.floor(parsed.months / xStep) + 1 },
    (_, i) => i * xStep
  );
  ...
  const projectedValue = moderateScenario ? moderateScenario.finalValue : 0;
    ...
    <div className="space-y-8 animate-fade-in-up">
            ...
            <div key={s.label} className="glass-card-3d p-6">
              <div className="flex items-center gap-2 mb-3">
                ...
                <p className="text-muted-foreground text-sm font-medium">
                  {s.label} ({(s.annualRate * 100).toFixed(0)}%)
                </p>
              </div>
              <p className="text-foreground text-3xl font-bold tabular-nums">
                {formatPrice(finalVal)}
              </p>
              <p
                className={`text-sm font-medium mt-1 ${
                  profit >= 0 ? 'price-up' : 'price-down'
                }`}
              >
                {profit >= 0 ? '+' : ''}
      ...
      <div className="glass-card-3d p-5">
        ...
        >
          ...
          {/* Area fills (draw conservative first, then moderate, then aggressive, then custom) */}
          {[2, 0, 3, 1].map((idx) => {
            const c = curves[idx];
            if (!c) return null;
            return (
        ...
        <div className="glass-card-3d p-6 text-center">
          ...
          <p
            className={`text-2xl font-bold tabular-nums ${
              projectedProfit >= 0 ? 'price-up' : 'price-down'
            }`}
          >
            {projectedProfit >= 0 ? '+' : ''}
