...
function projectDCA(initial: number, monthly: number, months: number, annualRate: number): number[] {
...
export default function ProfitProjections() {
  ...
  const moderateScenario = curves[1];
      ...
      <div className="glass-card-3d p-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
          <div className="space-y-2">
            ...
            <Input type="number" min="0" value={initial} onChange={(e) => setInitial(e.target.value)} className="bg-transparent border-white/10 text-foreground" />
          </div>
          <div className="space-y-2">
            <Label className="text-muted-foreground text-sm">Monthly DCA ($)</Label>
            <Input type="number" min="0" value={monthly} onChange={(e) => setMonthly(e.target.value)} className="bg-transparent border-white/10 text-foreground" />
          </div>
          <div className="space-y-2">
            <Label className="text-muted-foreground text-sm">Period (months)</Label>
            <Input type="number" min="1" max="120" value={months} onChange={(e) => setMonths(e.target.value)} className="bg-transparent border-white/10 text-foreground" />
          </div>
          <div className="space-y-2">
            <Label className="text-muted-foreground text-sm">Expected Annual Return (%)</Label>
            <Input type="number" step="0.5" value={customReturn} onChange={(e) => setCustomReturn(e.target.value)} className="bg-transparent border-white/10 text-foreground" />
          </div>
        </div>
      </div>
          ...
          return (
            <div key={s.label} className="glass-card-3d p-6">
              ...
              <p className={`text-sm font-medium mt-1 ${profit >= 0 ? 'price-up' : 'price-down'}`}>
                {profit >= 0 ? '+' : ''}{formatPrice(profit)} profit
              </p>
            </div>
          );
        })}
      ...
      <div className="glass-card-3d p-6">
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="flex flex-wrap gap-4 text-xs">
            ...
            <div className="flex items-center gap-1.5">
              <span className="w-3 h-0.5" style={{ borderTop: '2px dashed rgba(255,255,255,0.4)' }} />
        ...
        <svg viewBox={`0 0 ${CHART_W} ${CHART_H}`} className="w-full" preserveAspectRatio="xMidYMid meet">
          ...
          {investedY > PAD_T && investedY < PAD_T + PLOT_H && (
            <line x1={PAD_L} y1={investedY} x2={PAD_L + PLOT_W} y2={investedY} stroke="rgba(255,255,255,0.25)" strokeDasharray="6 4" strokeWidth="1" />
          )}
          {[2, 0, 3, 1].map((idx) => {
            const c = curves[idx];
            if (!c) return null;
            return <path key={c.label} d={buildAreaPath(c.curve)} fill={`url(#${c.gradientId})`} />;
          })}
          {curves.map((c) => (
            <path key={`line-${c.label}`} d={buildLinePath(c.curve)} fill="none" stroke={c.color} strokeWidth="2" strokeLinejoin="round" />
          ))}
        </svg>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-5">
        ...
        <div className="glass-card-3d p-6 text-center">
          ...
          <p className={`text-2xl font-bold tabular-nums ${projectedProfit >= 0 ? 'price-up' : 'price-down'}`}>
            {projectedProfit >= 0 ? '+' : ''}{formatPrice(projectedProfit)}
          </p>
        </div>
      </div>
    </div>
