...
function generateMockResult(
  ...
  for (let d = 1; d <= days; d++) {
    ...
    equityCurve.push(
      equityCurve[equityCurve.length - 1] *
        (1 + dailyReturn + (Math.random() - 0.5) * 0.005)
    );

    if (equityCurve[equityCurve.length - 1] > peak) {
      peak = equityCurve[equityCurve.length - 1];
    }
    ...
    if (Math.random() < 0.12) {
      ...
      const pnl = isBuy
        ? undefined
        : lastTrade
          ? (currentPrice - lastTrade.price) * (tradeAmount / currentPrice)
          : undefined;

      if (pnl !== undefined && pnl > 0) wins++;

      trades.push({
        date: new Date(Date.now() - (days - d) * 86400000)
          .toISOString()
          .split('T')[0],
        type: isBuy ? 'BUY' : 'SELL',
        price: currentPrice,
  ...
  const totalReturn =
    ((equityCurve[equityCurve.length - 1] - capital) / capital) * 100;
  ...
  return {
    ...
    sharpeRatio:
      (totalReturn / Math.max(maxDD, 0.01)) * 0.4 + Math.random() * 0.3,
    winRate,
    ...
    <div className="animate-fade-in-up space-y-6">
      ...
      {isRunning && (
        <div className="glass-card-3d p-12 text-center">
          ...
          <p className="text-foreground font-medium">Running backtest...</p>
          <p className="text-muted-foreground text-sm mt-1">
            Simulating {strategy} on {coin} over {period}
          </p>
        </div>
      )}
        ...
        >
          ...
          <div className="glass-card-3d p-5">
            ...
            <div className="w-full overflow-x-auto">
              <svg
                viewBox={`0 0 ${result.equityCurve.length * 4} 200`}
              ...
              >
                ...
                <path
                  d={
                    `M 0 ${200 - ((result.equityCurve[0] - minCurve) / curveRange) * 180 - 10} ` +
                    result.equityCurve
                      .map(
                        (v, i) =>
                          `L ${i * 4} ${200 - ((v - minCurve) / curveRange) * 180 - 10}`
                      )
                      .join(' ') +
                    ` L ${result.equityCurve.length * 4} 200 L 0 200 Z`
                  }
                  fill="url(#equityGrad)"
                />
                {/* Line */}
                ...
                {/* End dot */}
                <circle
                  cx={(result.equityCurve.length - 1) * 4}
                  cy={
                    200 -
                    ((result.equityCurve[result.equityCurve.length - 1] - minCurve) /
                      curveRange) *
                      180 -
                      10
                  }
