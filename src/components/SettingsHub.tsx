...
import {
  User, Mail, Palette, Bell, Database, Key, Info,
  Download, Trash2, Plus, Copy, Check, Eye, EyeOff,
...
export default function SettingsHub() {
  ...
  const [notifications, setNotifications] = useState({
    priceAlerts: true, tradeSignals: true, newsUpdates: false, system: true,
  });
  ...
  const handleExportData = () => {
    ...
    a.href = url; a.download = 'zcapital-export.json'; a.click();
    ...
    <div className="space-y-8 p-6 animate-fade-in-up">
      ...
      <div className="glass-card-3d p-8 animate-scale-in">
        <div className="flex items-center gap-3 mb-6">
          <div className="p-2 rounded-lg bg-teal-500/10"><User className="h-5 w-5 text-teal-400" /></div>
          <h2 className="text-xl font-semibold text-foreground">Profile</h2>
        </div>
        <div className="flex flex-col md:flex-row gap-8">
          <div className="flex flex-col items-center gap-3">
            <div className="w-24 h-24 rounded-full bg-gradient-to-br from-teal-400 to-emerald-500 flex items-center justify-center text-3xl font-bold text-white shadow-lg shadow-teal-500/20">AM</div>
          ...
          <div className="flex-1 grid grid-cols-1 md:grid-cols-2 gap-5">
            <div className="space-y-2">
              ...
              <div className="relative">
                ...
                <Input id="name" value={profileName} onChange={(e) => setProfileName(e.target.value)} className="pl-10 bg-white/[0.04] border-white/[0.08] text-foreground" />
              </div>
            </div>
            <div className="space-y-2">
              ...
              <div className="relative">
                ...
                <Input id="email" type="email" value={profileEmail} onChange={(e) => setProfileEmail(e.target.value)} className="pl-10 bg-white/[0.04] border-white/[0.08] text-foreground" />
              </div>
            </div>
            <div className="md:col-span-2 flex justify-end pt-2">
              <Button className="gradient-bg-4 text-foreground font-semibold hover:opacity-90 transition-opacity">Save Changes</Button>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="glass-card p-8 animate-fade-in-up" style={{ animationDelay: '0.1s' }}>
        <div className="flex items-center gap-3 mb-6">
          <div className="p-2 rounded-lg bg-amber-500/10"><Palette className="h-5 w-5 text-amber-400" /></div>
        ...
        <div className="flex flex-wrap gap-6">
          {GRADIENT_PRESETS.map((preset) => (
            <button key={preset.idx} onClick={() => setActiveGradient(preset.idx)} className="flex flex-col items-center gap-3 group">
              <div className={`w-14 h-14 rounded-full transition-all duration-300 ${activeGradient === preset.idx ? 'ring-2 ring-foreground ring-offset-2 ring-offset-[#0a0b0f] scale-110' : 'hover:scale-105'}`} style={{ background: preset.css }} />
              <span className={`text-xs font-medium transition-colors ${activeGradient === preset.idx ? 'text-foreground' : 'text-muted-foreground group-hover:text-foreground'}`}>{preset.name}</span>
              {activeGradient === preset.idx && <CheckCircle2 className="h-4 w-4 text-emerald-400 -mt-2" />}
            </button>
          ))}
        </div>
      </div>
      ...
      <div className="glass-card p-8 animate-fade-in-up" style={{ animationDelay: '0.15s' }}>
        <div className="flex items-center gap-3 mb-6">
          <div className="p-2 rounded-lg bg-cyan-500/10"><Bell className="h-5 w-5 text-cyan-400" /></div>
      ...
      <div className="glass-card p-8 animate-fade-in-up" style={{ animationDelay: '0.2s' }}>
        <div className="flex items-center gap-3 mb-6">
          <div className="p-2 rounded-lg bg-violet-500/10"><Database className="h-5 w-5 text-violet-400" /></div>
          <h2 className="text-xl font-semibold text-foreground">Data Management</h2>
        </div>
        <div className="flex flex-wrap gap-4">
          <Button variant="outline" onClick={handleExportData} className="border-white/[0.1] text-foreground hover:bg-white/[0.05] hover:text-foreground">
            <Download className="h-4 w-4 mr-2" /> Export Data
          </Button>
          <Button variant="outline" onClick={handleClearCache} className="border-red-500/20 text-red-400 hover:bg-red-500/10 hover:text-red-300">
      ...
      <div className="glass-card p-8 animate-fade-in-up" style={{ animationDelay: '0.25s' }}>
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center gap-3">
            <div className="p-2 rounded-lg bg-amber-500/10"><Key className="h-5 w-5 text-amber-400" /></div>
          ...
          <Dialog open={addKeyDialogOpen} onOpenChange={setAddKeyDialogOpen}>
            ...
            <DialogContent className="glass-card border-white/10 sm:max-w-md">
              <DialogHeader>
                ...
                <DialogDescription className="text-muted-foreground">Generate a new API key for programmatic access</DialogDescription>
              </DialogHeader>
              <div className="space-y-4 py-4">
                ...
                <Button className="w-full gradient-bg-1 text-foreground font-semibold hover:opacity-90 transition-opacity">Generate Key</Button>
              </div>
            </DialogContent>
          </Dialog>
        </div>
        <div className="space-y-3 stagger-children">
          {mockApiKeys.map((apiKey) => (
            <div key={apiKey.id} className="flex items-center gap-4 p-4 rounded-lg bg-white/[0.03] hover:bg-white/[0.05] transition-colors">
              <div className="p-2 rounded-lg bg-white/[0.05] shrink-0"><Key className="h-4 w-4 text-amber-400" /></div>
      ...
      <div className="glass-card p-8 animate-fade-in-up" style={{ animationDelay: '0.3s' }}>
        <div className="flex items-center gap-3 mb-6">
          <div className="p-2 rounded-lg bg-emerald-500/10"><Info className="h-5 w-5 text-emerald-400" /></div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div className="p-4 rounded-lg bg-white/[0.03]">
            ...
            <div className="flex items-center gap-3 mt-1">
              <button className="text-sm text-foreground hover:underline flex items-center gap-1">Documentation <ExternalLink className="h-3 w-3" /></button>
              <button className="text-sm text-foreground hover:underline flex items-center gap-1">Support <ExternalLink className="h-3 w-3" /></button>
            </div>
          </div>
        </div>
      </div>
